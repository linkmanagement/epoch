...
// Account info for password "sebastian"  - 0 LMMCOIN amount
var Saccount = {
...
describe('Account', function() {
    ...
    it(test + '. Testing /accounts/getBalance but sending INVALID ADDRESS. Expecting error',function(done){
        node.api.get('/accounts/getBalance?address=thisIsNOTALMMcoinAddress')
            .set('Accept', 'application/json')
            .expect('Content-Type', /json/)
            .expect(200)
            .end(function (err, res){
                ...
                node.expect(res.body).to.have.property("success").to.be.false;
                node.expect(res.body).to.have.property("error");
                // expect(res.body.error).to.contain("Provide valid LMMcoin address");
                done();
            });
    });
    ...
    it(test + '. Testing /accounts/getPublicKey but SENDING INVALID ADDRESS. Expecting error',function(done){
        node.api.get('/accounts/getPublicKey?address=thisIsNOTALMMcoinAddress')
            .set('Accept', 'application/json')
            .expect('Content-Type', /json/)
            .expect(200)
            .end(function (err, res){
    ...
    it(test + '. Testing /accounts but SENDING INVALID ADDRESS. Expecting error',function(done){
        node.api.get('/accounts?address=thisIsNOTAValidLMMcoinAddress')
            .set('Accept', 'application/json')
            .expect('Content-Type', /json/)
            .expect(200)
            .end(function (err, res){
