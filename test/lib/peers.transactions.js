...
describe("Peers transactions", function () {
  it("create transaction. should return ok", function (done) {
    var transaction = node.lmmcoin.transaction.createTransaction("1C", 1, node.peers_config.account);
  ...
  it("create transaction with recipient and then change it to verify signature. should return not ok", function (done) {
    var transaction = node.lmmcoin.transaction.createTransaction("12C", 1, node.peers_config.account);
  ...
  it("create transaction with no balance on sender. should return not ok", function (done) {
      var transaction = node.lmmcoin.transaction.createTransaction("1C", 1, "randomstring");
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
  ...
  it("send transaction with very large amount and genesis block id. should return no ok", function (done) {
      var transaction = node.lmmcoin.transaction.createTransaction("12C", 10000000000000000, node.peers_config.account);
  ...
  it("send very large number in transaction. should return not ok", function (done) {
      var transaction = node.lmmcoin.transaction.createTransaction("12C", 184819291270000000012910218291201281920128129, node.peers_config.account);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
  ...
  it("send float number in transaction, should return not ok", function (done) {
      var transaction = node.lmmcoin.transaction.createTransaction("12C", 1.3, node.peers_config.account);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
