...
describe("Peer contacts", function () {
  ...
  it("Add not exists account to contacts. Should return not ok", function (done) {
      var transaction = node.lmmcoin.contact.createContact(account.password, "5819218109212912C");
  ...
  it("Add account to contact with minus. Should be not ok", function (done) {
    var transaction = node.lmmcoin.contact.createContact(account.password, "-" + node.peers_config.address);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
