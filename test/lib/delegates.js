...
console.log("Random LMMCOIN is: " + (node.LMMCOIN / node.normalizer));
...
describe('Delegates', function() {
    ...
    describe('Vote and Register delegate attempts from account with 0 LMMCOIN', function() {

        test += 1;
        it(test + '. We attempt to upVote a delegate from the new random account. We expect error (account should have 0 LMMCOIN)',function(done){
        ...
        test += 1;
        it(test + '. We attempt to downVote a delegate from the new random account. We expect error (account should have 0 LMMCOIN)',function(done){
            node.onNewBlock(function(err) {
                node.api.put('/accounts/delegates')
                    .set('Accept', 'application/json')
                    .send({
        ...
        test += 1;
        it(test + '. We attempt to register as delegate from random account. Since we have 0 LMMCOIN, we expect error',function(done){
    ...
    describe('upVoting and downVoting',function() {
        ...
        before(function(done){
            // Send random LMMCOIN amount from foundation account to Random account
            node.api.put('/transactions')
                ...
                .end(function (err, res) {
                    ...
                    if (res.body.success == true && res.body.transactionId != null){
                        ...
                        Raccount.amount += node.LMMCOIN;
                    }
                    else{
                        console.log("Transaction failed or transactionId is null");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + node.LMMCOIN + ", recipientId: " + Raccount.address);
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
        ...
        before(function (done) {
            // Check that Raccount has the LMMCOIN we sent
                ...
                node.api.post('/accounts/open')
                    ...
                    .end(function (err, res) {
                        ...
                        if (res.body.success == true && res.body.account != null){
                            node.expect(res.body.account.balance).to.be.equal(node.LMMCOIN);
                        }
                        else{
    ...
    describe('Get Delegates list',function() {
        ...
        it(test + '. We attempt to get delegate via address parameter but we send INVALID ADDRESS. We expect error',function(done){
            node.api.get('/accounts/delegates?address=NOTaLMMcoinAddress')
                .set('Accept', 'application/json')
                .expect('Content-Type', /json/)
                .expect(200)
                .end(function (err, res) {
