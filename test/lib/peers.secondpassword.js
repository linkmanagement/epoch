...
describe("Peers second signature transactions", function () {
  it("Send second signature from account that doesn't have it. Should return not ok", function (done) {
    var transaction = node.lmmcoin.transaction.createTransaction("1C", 1, node.peers_config.account, account.secondPassword);
  ...
  it("Send second signature from account that have no funds. Should return not ok", function (done) {
    var transaction = node.lmmcoin.signature.createSignature(node.randomPassword(), node.randomPassword());
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            ...
            node.onNewBlock(function (err) {
              ...
              var transaction = node.lmmcoin.signature.createSignature(account.password, account.secondPassword);
              transaction.fee = node.Fees.secondPasswordFee;
  ...
  it("Test transaction with second signature. Should return ok", function (done) {
    var transaction = node.lmmcoin.transaction.createTransaction("1C", 1, account.password, account.secondPassword);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
  ...
  it("Test transaction without second signature. Should return not ok", function (done) {
    var transaction = node.lmmcoin.transaction.createTransaction("1C", 1, account.password);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
  ...
  it("Test transaction with fake second signature. Should return not ok", function (done) {
    ...
    transaction.id = node.lmmcoin.crypto.getId(transaction);
    node.peer.post('/transactions')
      .set('Accept', 'application/json')
      .set('version',node.version)
      .set('share-port',1)
      ...
      .end(function (err, res) {
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            ...
            node.onNewBlock(function (err) {
              node.expect(err).to.be.not.ok;
              var transaction = node.lmmcoin.signature.createSignature(account2.password, account2.secondPassword);
              transaction.fee = node.Fees.secondPasswordFee;
              ...
              node.peer.post('/transactions')
                ...
                .expect(200)
                .end(function (err, res) {
                  var sendTransaction = node.lmmcoin.transaction.createTransaction("1C", 1, account.password);
                  node.peer.post('/transactions')
                    .set('Accept', 'application/json')
                    .set('version',node.version)
                    .set('share-port',1)
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            node.onNewBlock(function (err) {
              ...
              var sendTransaction = node.lmmcoin.transaction.createTransaction("1C", 1, account3.password);
              node.peer.post('/transactions')
                .set('Accept', 'application/json')
                .set('version',node.version)
                .set('share-port',1)
                ...
                .end(function (err, res) {
                  ...
                  var transaction = node.lmmcoin.signature.createSignature(account3.password, account3.secondPassword);
                  transaction.fee = node.Fees.secondPasswordFee;
