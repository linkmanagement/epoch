...
describe("Peers votes", function () {
    ...
    node.api.get('/delegates/voters?publicKey=' + node.peers_config.publicKey)
      ...
      .end(function (err, res) {
        ...
        if (alreadyVoted == 1 && alreadyRemoved == 0) {
            var transaction = node.lmmcoin.vote.createVote(node.peers_config.account, ["+badf44a77df894ccad87fa62bac892e63e5e39fd972f6a3e6e850ed1a1708e98", "+9062a3b2d585be13b66e705af3f40657a97d0e4a27ec56664e05cdb5c953b0f6"]);
        }
        else if (alreadyVoted == 1) {
            var transaction = node.lmmcoin.vote.createVote(node.peers_config.account, ["+badf44a77df894ccad87fa62bac892e63e5e39fd972f6a3e6e850ed1a1708e98"]);
        }
        else if (alreadyRemoved == 0) {
            var transaction = node.lmmcoin.vote.createVote(node.peers_config.account, ["+9062a3b2d585be13b66e705af3f40657a97d0e4a27ec56664e05cdb5c953b0f6"]);
        }
        if (alreadyVoted == 1 || alreadyRemoved == 0){
    ...
    node.onNewBlock(function (err) {
      ...
      node.peer.post('/transactions')
        ...
        .end(function (err, res) {
          ...
          node.expect(res.body).to.have.property("success").to.be.true;
          var transaction2 = node.lmmcoin.vote.createVote(node.peers_config.account, ["+9062a3b2d585be13b66e705af3f40657a97d0e4a27ec56664e05cdb5c953b0f6"]);
          node.peer.post('/transactions')
            .set('Accept', 'application/json')
            .set('version', node.version)
            .set('share-port', 1)
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            node.onNewBlock(function (err) {
              ...
              var transaction = node.lmmcoin.delegate.createDelegate(account.password, account.username);
  ...
  it("Vote for created delegate. Should return not ok", function (done) {
      var transaction = node.lmmcoin.vote.createVote(node.peers_config.account, ["+" + account.publicKey]);
    node.onNewBlock(function (err) {
      node.expect(err).to.be.not.ok;
      node.peer.post('/transactions')
        .set('Accept', 'application/json')
