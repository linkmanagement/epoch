...
// Account info for a RANDOM account (which we create later) - 0 LMMCOIN amount | Will act as delegate
...
describe('Transactions', function() {
    ...
    before(function (done) {
        // Send to LMMCOIN to account 1 address
        setTimeout(function() {
            randomLMMcoin = node.randomizeLMMcoin();
            expectedFee = node.expectedFee(randomLMMcoin);
            node.api.put('/transactions')
                .set('Accept', 'application/json')
                .send({
                    secret: node.Faccount.password,
                    amount: randomLMMcoin,
                ...
                .end(function (err, res) {
                    ...
                    if (res.body.success == true && res.body.transactionId != null) {
                        console.log('Sent to ' + Account1.address + ' ' + (randomLMMcoin / node.normalizer) + ' LMMCOIN');
                        console.log('Expected fee (paid by sender): ' + expectedFee / node.normalizer + ' LMMCOIN');
                        Account1.transactions.push(transactionCount);
                        transactionCount += 1;
                        totalTxFee += (expectedFee / node.normalizer);
                        Account1.balance += randomLMMcoin;
                        transactionList[transactionCount - 1] = {
                            'sender': node.Faccount.address,
                            'recipient': Account1.address,
                            'brutoSent': (randomLMMcoin + expectedFee) / node.normalizer,
                            'fee': expectedFee / node.normalizer,
                            'nettoSent': randomLMMcoin / node.normalizer,
                            'txId': res.body.transactionId,
                            'type':node.TxTypes.SEND
                        }
                    }
                    else{
                        console.log("Sending LMMCOIN to Account1 failed.");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + randomLMMcoin + ", recipientId: " + Account1.address );
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
    ...
    before(function (done) {
        ...
        setTimeout(function() {
            ...
            node.api.put('/transactions')
                ...
                .end(function (err, res) {
                    console.log(JSON.stringify(res.body));
                    console.log('We send the LMMCOIN from foundation account to account. Recipient is: ' + Account2.address);
                    console.log('Sent to ' + Account2.address + ' ' + (randomLMMcoin / node.normalizer) + ' LMMCOIN');
                    console.log('Expected fee (paid by sender): ' + expectedFee / node.normalizer + ' LMMCOIN');
                    node.expect(res.body).to.have.property("success").to.be.true;
                    if (res.body.success == true && res.body.transactionId != null) {
                        ...
                        transactionCount += 1;
                        totalTxFee += (expectedFee / node.normalizer);
                        Account2.balance += randomLMMcoin;
                        transactionList[transactionCount - 1] = {
                            'sender': node.Faccount.address,
                        ...
                        console.log("Sending LMMCOIN to Account2 failed.");
                        console.log("Sent: secret: " + node.Faccount.password + ", amount: " + randomLMMcoin + ", recipientId: " + Account2.address );
                        node.expect("TEST").to.equal("FAILED");
                    }
                    done();
                });
      ...
      node.expect(err).to.be.not.ok;
        ...
        describe('/transactions', function () {
            ...
            it(test + '. Attempting to get transactions list. Sending INVALID FIELDS. Expecting error', function (done) {
                var senderId = "notAReadAddress", blockId = 'about5', recipientId = 'LMMcoinLIOnair3', limit = 'aLOT', offset = 'Boris', orderBy = 't_blockId:asc';
          ...
          node.api.get('/transactions?blockId=' + blockId + '&senderId=' + senderId + '&recipientId=' + recipientId + '&limit=' + limit + '&offset=' + offset + '&orderBy=' + orderBy)
            ...
            test += 1;
            it(test + '. We send LMMCOIN from Account 1 (' + Account1.password + ') to Account 2 (' + Account2.address + ') - valid data. We expect success',function(done){
                node.onNewBlock(function(err) {
          node.expect(err).to.be.not.ok;
            ...
            test += 1;
            it(test + '. We try to send NEGATIVE LMMCOIN VALUE from Account 1 to Account 2. We expect error',function(done){
                    amountToSend = -100000000;
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
                        .send({
            ...
            test += 1;
            it(test + '. We try to send INVALID LMMCOIN VALUE from Account 1 to Account 2. We expect error',function(done){
                amountToSend = 1.2;
                node.api.put('/transactions')
                    .set('Accept', 'application/json')
                    .send({
            ...
            test += 1;
            it(test + '. We try to send 100% LMMCOIN as amout from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send 0 LMMCOIN from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY LARGE LMMCOIN NUMBER from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY LARGE NEGATIVE LMMCOIN NUMBER of LMMCOIN from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY SMALL LMMCOIN NUMBER (0.00000001) from Account 1 to Account 2. We get success',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send VERY SMALL NEGATIVE LMMCOIN NUMBER (0.00000001) from Account 1 to Account 2. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
            ...
            test += 1;
            it(test + '. We try to send LMMCOIN WITHOUT PASSWORD. We expect error',function(done){
                amountToSend = 100000000;
                node.api.put('/transactions')
                    .set('Accept', 'application/json')
                    .send({
            ...
            test += 1;
            it(test + '. We try to send LMMCOIN WITHOUT RECIPIENT. We expect error',function(done){
                amountToSend = 100000000;
                node.api.put('/transactions')
                    .set('Accept', 'application/json')
                    .send({
            ...
            test += 1;
            it(test + '. We attempt to created 2nd password from account with 0 LMMCOIN. We expect error',function(done){
                this.timeout(5000);
                setTimeout(function(){
            ...
            test += 1;
            it(test + '. We try to send LMMCOIN WITHOUT SECOND PASSWORD. We expect error',function(done){
                amountToSend = 100000000;
                node.onNewBlock(function(err){
          node.expect(err).to.be.not.ok;
          ...
          node.api.put('/transactions')
            ...
            test += 1;
            it(test + '. We try to send LMMCOIN WITH SECOND PASSWORD but NO SECRET. We expect error',function(done){
                amountToSend = 100000000;
                this.timeout(5000);
                setTimeout(function(){
                    node.api.put('/transactions')
        ...
        console.log(Account1);
          ...
          node.expect(err).to.be.not.ok;
            ...
            test += 1;
            it(test + '. We try to SEND LMMCOIN to USERNAME. We expect success',function(done){
                node.onNewBlock(function(){
                    amountToSend = 100000000;
                    node.api.put('/transactions')
                        .set('Accept', 'application/json')
