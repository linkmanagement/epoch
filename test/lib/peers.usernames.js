...
describe("Peers usernames", function () {
  it("Register username on new account and then try to register another username. Should return not ok", function (done) {
      var transaction = node.lmmcoin.username.createUsername(node.peers_config.account, node.randomDelegateName());
    ...
    node.peer.post('/transactions')
      ...
      .end(function (err, res) {
        ...
        transaction = node.lmmcoin.username.createUsername(node.peers_config.account, node.randomAccount());
        transaction.fee = node.Fees.usernameFee;
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            node.onNewBlock(function (err) {
              console.log("We sent LMMCOIN to account. We got answer: " + JSON.stringify(res.body));
              node.expect(err).to.be.not.ok;
              var transaction = node.lmmcoin.delegate.createDelegate(account.password, node.randomDelegateName());
              ...
              node.peer.post('/transactions')
                ...
                .end(function (err, res) {
                  ...
                  node.expect(res.body).to.have.property("success").to.be.true;
                  transaction = node.lmmcoin.username.createUsername(account.password, node.randomDelegateName());
                  transaction.fee = node.Fees.usernameFee;
      ...
      .end(function (err, res) {
        ...
        node.api.put('/transactions')
          ...
          .end(function (err, res) {
            node.onNewBlock(function (err) {
              node.expect(err).to.be.not.ok;
              var transaction = node.lmmcoin.username.createUsername(account2.password, node.randomDelegateName());
              transaction.fee = node.Fees.usernameFee;
              ...
              node.peer.post('/transactions')
                ...
                .end(function (err, res) {
                  ...
                  node.onNewBlock(function () {
                      transaction = node.lmmcoin.delegate.createDelegate(account2.password, node.randomDelegateName());
                    transaction.fee = node.Fees.delegateRegistrationFee;
