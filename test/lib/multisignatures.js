...
var NoLMMcoinAccount = node.randomAccount();
NoLMMcoinAccount.name = "noLMMcoin";
...
function openAccount (account, i){
        node.api.post('/accounts/open')
            ...
            .end(function (err, res) {
                ...
                else if (account.name == "noLMMcoin"){
                    NoLMMcoinAccount.address = res.body.account.address;
                    NoLMMcoinAccount.publicKey = res.body.account.publicKey;
                }
...
// Used for sending LMMCOIN to accounts
var accountSendTurn = 0;

function sendLMMcoin (account, i){
    node.onNewBlock(function(err) {
        var randomLMMcoin = node.randomizeLMMcoin();
        node.api.put('/transactions')
            ...
            .end(function (err, res) {
                ...
                console.log("Sending " + randomLMMcoin + " LMMCOIN to " + account.address);
                node.expect(res.body).to.have.property("success").to.be.true;
                if (res.body.success == true && i != null){
                    Accounts[i].balance = randomLMMcoin / node.normalizer;
                }
            });
    });
}

function sendLMMcoinfromMultisigAccount (amount, recipient){
    node.api.put('/transactions')
        .set('Accept', 'application/json')
        .send({
        ...
        .end(function (err, res) {
            console.log(JSON.stringify(res.body));
            console.log("Sending " + amount + " LMMCOIN to " + recipient);
            node.expect(res.body).to.have.property("success").to.be.true;
...
describe('Multisignatures', function() {
    ...
    before(function (done) {
        ...
        openAccount(NoLMMcoinAccount, null);
    ...
    before(function (done) {
       for (var i = 0; i < (Accounts.length); i++){
           if(Accounts[i] != null){
               sendLMMcoin(Accounts[i], i);
           }
       }
       sendLMMcoin(MultisigAccount, null);
       done();
    });
    ...
    describe('Create Multisignature Account', function() {
        ...
        test += 1;
        it(test + '. Account with 0 LMMCOIN. We expect error', function (done) {
            node.api.put('/multisignatures')
                .set('Accept', 'application/json')
                .send({
                    secret: NoLMMcoinAccount.password,
                    lifetime: 1,
                    min: requiredSignatures,
                    keysgroup: Keys
                })
    ...
    describe('Sign Pending Multisignature', function() {
        ...
        test += 1;
        it(test + '. Try to send LMMCOIN FROM multisignature account (confirmations still pending). We expect success',function(done) {
            node.onNewBlock(function (err) {
                sendLMMcoinfromMultisigAccount(100000000, node.Faccount.address);
                done();
            });
        });
    });
